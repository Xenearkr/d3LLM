...
def main():
        ...
        if trajectory_dataset is None:
            ...
            def preprocess_trajectory_sample(examples):
                ...
                for traj in examples["trajectory"]:
                    ...
                    assert isinstance(traj, list), f"Invalid trajectory format: {type(traj)}"
                    if len(traj) > 1:
                        traj = [traj[0]]
                    assert len(traj) >= 1 and isinstance(traj[0], list), f"Invalid trajectory format: {type(traj)}"
                    decode_order = traj[0]
    ...
    # trajectory_dataset = [trajectory_dataset[0]] # new，过滤多余output
    ...
    cache_params = {
        "dataset_name": "trajectory_dataset_apps",
