...
"""
独立副本：由 d3llm_dream_train.py 复制而来，不 import 该文件，便于单独复现。

与 d3llm_dream_train.py 的差异仅在于：
- 默认配置路径 d3llm_train_finetune_hf.yaml；备份 d3llm_train_finetune_hf_used.yaml
- 起点模型为 yaml 中的 HF id（如 d3LLM/d3LLM_Dream_Coder）
- distillation.trajectory_dataset_paths（列表）或 trajectory_dataset_path：HF hub 或本地目录；
  多源 concatenate_datasets；文本用 question + text_response_field（默认 llm_answer）
- 轨迹/词元缓存键含 finetune_hf，避免与原版 cache 冲突
- forward_process_with_trajectory：轨迹长度与 batch 序列长度对齐（collator 512+max(prompt) 可能 > max_length）
"""
...
def _is_local_dataset_dir(p: str) -> bool:
    return os.path.isdir(_expand_path(p))


def load_one_trajectory_source(path_or_id: str):
    if _is_local_dataset_dir(path_or_id):
        return load_from_disk(_expand_path(path_or_id))
    return load_dataset(path_or_id, split="train")


def normalize_trajectory_sources(distill: Dict[str, Any]) -> List[str]:
    paths = distill.get("trajectory_dataset_paths")
    if paths is None:
        single = distill.get("trajectory_dataset_path")
        if not single:
            return []
...
def concat_trajectory_datasets(sources: List[str]):
    parts = [load_one_trajectory_source(s) for s in sources]
    if len(parts) == 1:
        return parts[0]
...
def main():
    ...
    config_path = os.path.join(script_dir, "d3llm_train_finetune_hf.yaml")
    if len(sys.argv) > 1 and str(sys.argv[1]).endswith((".yaml", ".yml")):
        config_path = os.path.abspath(sys.argv.pop(1))
    config = load_config(config_path)

    # Override config from command line args
    config = override_config(config, sys.argv[1:])

    # Save modified config for backup
    used_config_path = os.path.join(script_dir, "d3llm_train_finetune_hf_used.yaml")
    ...
    sources = normalize_trajectory_sources(distill_config)
    if not sources:
        raise ValueError("请在 distillation 中配置 trajectory_dataset_paths 或 trajectory_dataset_path")

    max_length = distill_config.get("max_length", 1000)
    num_proc = distill_config.get("num_proc", 8)
    response_field = distill_config.get("text_response_field", "llm_answer")

    traj_cache_dir = os.path.join(script_dir, "cache", "trajectory_finetune_hf")
    os.makedirs(traj_cache_dir, exist_ok=True)
    traj_cache_key = hashlib.md5(
        (json.dumps(sources, sort_keys=True) + f"_maxlen{max_length}").encode()
    ).hexdigest()
    traj_cache_file = os.path.join(traj_cache_dir, f"traj_preprocessed_{traj_cache_key}.pkl")

    trajectory_dataset = None
    if os.path.isfile(traj_cache_file):
        try:
            print(f"Loading trajectory cache: {traj_cache_file}")
            with open(traj_cache_file, "rb") as f:
                trajectory_dataset = pickle.load(f)
            print(f"Loaded {len(trajectory_dataset)} trajectory rows from cache")
        except Exception as e:
            print(f"Trajectory cache load failed: {e}")
            trajectory_dataset = None

    if trajectory_dataset is None:
        print(f"Loading trajectory sources: {sources}")
        trajectory_dataset = concat_trajectory_datasets(sources)
        if response_field not in trajectory_dataset.column_names:
            raise ValueError(
                f"字段 {response_field} 不在数据集中，当前列: {trajectory_dataset.column_names}"
            )
        ...
        try:
            with open(traj_cache_file, "wb") as f:
                pickle.dump(trajectory_dataset, f)
            print(f"Saved trajectory cache: {traj_cache_file}")
        except Exception as e:
            print(f"Warning: trajectory cache save failed: {e}")

    max_samples = distill_config.get("max_samples")
    if max_samples is not None and max_samples > 0:
        n = min(max_samples, len(trajectory_dataset))
        trajectory_dataset = trajectory_dataset.select(range(n))
        print(f"[Testing Mode] trajectory_dataset limited to {n} rows")

    text_ds = trajectory_dataset.remove_columns(
        [c for c in trajectory_dataset.column_names if c not in ("question", response_field)]
    ).rename_column(response_field, "llm_response")

    cache_dir = os.path.join(script_dir, "cache")
    os.makedirs(cache_dir, exist_ok=True)
    cache_params = {
        "finetune_hf_v2_prompt_cap": True,
        "sources": sources,
    ...
    cache_file_tokenized = os.path.join(cache_dir, f"tokenized_finetune_hf_{cache_key}.pkl")

    if os.path.exists(cache_file_tokenized):
        try:
            print(f"Loading tokenized cache: {cache_file_tokenized}")
            with open(cache_file_tokenized, "rb") as f:
                tokenized_dataset = pickle.load(f)
            print(f"Loaded {len(tokenized_dataset)} tokenized rows from cache")
        except Exception as e:
            print(f"Tokenized cache load failed: {e}")
    ...
    if tokenized_dataset is None:
        ...
        def format_example(example):
            ...
            for question, response in zip(example["question"], example["llm_response"]):
                ...
                answer_text = (response or "") + tokenizer.eos_token
        ...
        print("Formatting text from trajectory dataset...")
        formatted_dataset = text_ds.map(format_example, batched=True)
        ...
        def tokenize_function(examples, indices):
            ...
            # prompt_length 在 format 阶段按「未截断 prompt」计数；整条序列截断到 max_length 后，
            # 若仍用原值会导致 collator 里 512+max(prompt) >> 实际长度，把 batch pad 到数千 token 爆显存。
            capped_pl = [
                min(int(pl), len(ids)) for pl, ids in zip(examples["prompt_length"], tokenized["input_ids"])
            ]
            tokenized["prompt_lengths"] = capped_pl
        ...
        try:
            ...
            print(f"Saved tokenized cache: {cache_file_tokenized}")
        except Exception as e:
            print(f"Warning: tokenized cache save failed: {e}")
    ...
    class MaskDiffusionDataCollator:
        ...
        def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
            ...
            prompt_lengths = [int(f["prompt_lengths"]) for f in features]
            sample_indices = [f["sample_idx"] for f in features]
            base_lens = [ids.numel() for ids in input_ids]
            base = max(base_lens)
            pl_adj = [min(pl, bl) for pl, bl in zip(prompt_lengths, base_lens)]
            formula = 512 + max(pl_adj)
            target_length = min(self.max_seq_len, max(base, min(formula, self.max_seq_len)))

            pad_token_id = self.tokenizer.eos_token_id

            padded_input_ids = []
            for ids in input_ids:
                current_length = ids.numel()
            ...
            batch = {
                ...
                "prompt_lengths": torch.tensor(pl_adj, dtype=torch.long),
