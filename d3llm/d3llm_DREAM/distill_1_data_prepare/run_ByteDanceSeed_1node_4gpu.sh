...
GEN_LENGTH=512
BLOCK_LENGTH=32
OUTPUT_DIR="trajectory_data_1node_4gpu_ByteDanceSeed"
...
cleanup() {
  ...
  pkill -f "ByteDanceSeed_multinode.py.*--output_dir ${OUTPUT_DIR}" 2>/dev/null || true
  pkill -f "ByteDanceSeed_partly.py" 2>/dev/null || true
}
...
for RANK in 0 1 2 3; do
  ...
  "${PYTHON_BIN}" d3llm/d3llm_DREAM/distill_1_data_prepare/ByteDanceSeed_multinode.py \
    --num_gpus ${NUM_GPUS} \
    --steps ${STEPS} \
...
# chmod +x run_ByteDanceSeed_1node_4gpu.sh
# ./run_ByteDanceSeed_1node_4gpu.sh